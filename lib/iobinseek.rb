module BinSeek
  def binseek(bytes, seek_table = nil)
    ...
    return nil if (self.pos + encoded_bytes_size) > self.size
    if encoded_bytes_size == 1
      ord = encoded_bytes.ord
      byte = self.getbyte until (byte == ord) or self.eof?
      if (byte == ord)
        self.seek(-encoded_bytes_size, IO::SEEK_CUR)
      end
      return 0
    end
    seek_table = self.seek_table(encoded_bytes) if seek_table.nil?
    size_less_1 = encoded_bytes_size - 1
    encoded_bytes_last = encoded_bytes[size_less_1].ord
    size_minus_length = self.size - encoded_bytes_size
    while self.pos <= size_minus_length
      ...
      delta = encoded_bytes_size if delta.nil?
  ...
  def seek_table(encoded_bytes)
    ...
    encoded_bytes.size.times { |n| seek_table[encoded_bytes[n].ord] = -(n + 1) }
    seek_table
  end
end
